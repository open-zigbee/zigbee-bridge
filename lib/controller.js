    ...
    */
    ...
    reset(mode) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                return Promise.reject(new Error('Unknown reset mode.'));
            })
            .then(() => {
                this._resetting = false;
                if (this._nvChanged) {
    ...
    permitJoin(time, type) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                if (time > 255 || time < 0) {
                    return Promise.reject(new Error('Jointime can only range from  0 to 255.'));
                }
    ...
    endDeviceAnnceHdlr(data) {
        ...
        if (dev && dev.status === 'online') {
            ...
            this.getShepherd().emit('joining', {
                type: 'already_in_network',
                ieeeAddr: data.ieeeaddr,
            });
