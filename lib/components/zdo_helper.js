...
const zdoHelper = {
    ...
    getRequestType(reqName) {
        const meta = zdoReqRspMap[reqName];
        return meta ? meta.apiType : 'rspless';
    },

    generateEventOfRequest(reqName, valObj) {
        const meta = zdoReqRspMap[reqName];
        let evtName;

        if (!zdoHelper.hasAreq(reqName)) return;

        evtName = 'ZDO:' + meta.ind;

        if (meta.suffix.length === 0) return evtName;
    ...
    generateEventOfIndication(indName, msgData) {
