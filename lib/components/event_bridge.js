...
const debug = {
    msgHdlr: require('debug')('zigbee-bridge:msgHdlr'),
};

const bridge = {
    _areqEventBridge(controller, msg) {
        // msg: { subsys: 'ZDO', ind: 'endDeviceAnnceInd', data: { srcaddr: 63536, nwkaddr: 63536, ieeeaddr: '0x00124b0001ce3631', ... }
        // 'SYS:resetInd', 'SYS:osalTimerExpired'
        const mandatoryEvent = msg.subsys + ':' + msg.ind;

        // bridge to subsystem events, like 'SYS:resetInd', 'SYS:osalTimerExpired'
        controller.emit(mandatoryEvent, msg.data);
    ...
    _zdoIndicationEventBridge(controller, msg) {
        const payload = msg.data;
        const zdoEventHead = 'ZDO:' + msg.ind;
        let zdoBridgedEvent;

        if (msg.ind === 'stateChangeInd') {
            // this is a special event
            // Coord itself
            if (!payload.hasOwnProperty('nwkaddr')) {
    ...
    _sapiIndicationEventBridge(controller, msg) {
        const payload = msg.data;
