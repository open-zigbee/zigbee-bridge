...
af.send = function(srcEp, dstEp, cId, rawPayload, opt, callback) {
    ...
    let controller = af.controller;
    let areq = af.areq;
    let areqTimeout;
    let profId = srcEp.getProfId();
    let afParams;
    let afEventCnf;
    let apsAck = false;
    let senderEp;
    ...
    senderEp = srcEp.isLocal()
        ? srcEp
        : controller.getCoord().getDelegator(profId);
    ...
    areq.register(afEventCnf, deferred, (cnf) => {
        ...
        if (cnf.status === 0 || cnf.status === 'SUCCESS') {
            // success
        ...
        } else if (cnf.status === 0xb7 || cnf.status === 'APS_NO_ACK') {
            // ZApsNoAck period is 20 secs
            areq.reject(afEventCnf, new Error(errText + '183. APS no ack.'));
        } else if (cnf.status === 0xf0 || cnf.status === 'MAC_TRANSACTION_EXPIRED') {
    ...
    controller.request('AF', 'dataRequest', afParams)
        .then((rsp) => {
        ...
        .fail((err) => areq.reject(afEventCnf, err))
        .done();
...
af.sendExt = function(srcEp, addrMode, dstAddrOrGrpId, cId, rawPayload, opt, callback) {
    ...
    if (addrMode === ZSC.AF.addressMode.ADDR_GROUP || addrMode === ZSC.AF.addressMode.ADDR_BROADCAST) {
        // no ack
        controller.request('AF', 'dataRequestExt', afParamsExt)
            .then((rsp) => {
                if (rsp.status !== 0 && rsp.status !== 'SUCCESS') {
                    // unsuccessful
                    deferred.reject(new Error('AF data extend request failed, status code: ' + rsp.status + '.'));
                } else {
                    // Broadcast (or Groupcast) has no AREQ confirm back, just resolve this transaction.
...
af.zclFoundation = function(srcEp, dstEp, cId, cmd, zclData, cfg, callback) {
    ...
    frameCntl = {
        // command acts across the entire profile (foundation)
        frameType: 0,
        manufSpec: cfg.hasOwnProperty('manufSpec')
            ? cfg.manufSpec
            : 0,
        direction: cfg.hasOwnProperty('direction')
            ? cfg.direction
            : dir,
        // enable deafult response command
        disDefaultRsp: cfg.hasOwnProperty('disDefaultRsp')
            ? cfg.disDefaultRsp
            : 0,
    };
    ...
    af.send(srcEp, dstEp, cId, zclBuffer)
        .fail((err) => {
        ...
        .then((rsp) => {
            if (!mandatoryEvent) deferred.resolve(rsp);
        })
        .done();
...
af.zclClustersReq = function(dstEp, eventEmitter, callback) {
    ...
    let clusters = {};
    // [ 1, 2, 3, 4, 5 ]
    let clusterList = dstEp.getClusterList();
    // [ 1, 2, 3 ]
    let inClusterList = dstEp.getInClusterList();
    // [ 1, 3, 4, 5 ]
    let outClusterList = dstEp.getOutClusterList();
    // clusterAttrsRsps = [];  // { attr1: x }, ...
    let clusterAttrsReqs = [];
    ...
    _.forEach(clusterList, (cId) => {
        ...
        clusterAttrsReqs.push((clusters) => {
            return af.zclClusterAttrsReq(dstEp, cId)
                .then((attrs) => {
                    i++;
                    ...
                    clusters[cIdString].dir = _.includes(inClusterList, cId)
                        ? (clusters[cIdString].dir | 0x01)
                        : clusters[cIdString].dir;
                    clusters[cIdString].dir = _.includes(outClusterList, cId)
                        ? (clusters[cIdString].dir | 0x02)
                        : clusters[cIdString].dir;
    ...
    // all clusters
    clusterAttrsReqs.reduce((soFar, f) => soFar.then(f), Q(clusters))
        .then((clusters) => {
...
af.zclClusterAttrsReq = function(dstEp, cId, callback) {
    ...
    af.zclClusterAttrIdsReq(dstEp, cId)
        .then((attrIds) => {
            let readReq = [];
            let attrsReqs = [];
            let attributes = [];
            ...
            _.forEach(attrIds, (id) => {
                ...
                if (readReq.length === 5 || readReq.length === attrIdsLen) {
                    let req = _.cloneDeep(readReq);
                    attrsReqs.push((attributes) => {
...
af.zclClusterAttrIdsReq = function(dstEp, cId, callback) {
    ...
    const discAttrs = function(startAttrId, defer) {
        const opts = {startAttrId: startAttrId, maxAttrIds: 240};
        af.zclFoundation(dstEp, dstEp, cId, 'discover', opts)
            .then((discoverRsp) => {
                // discoverRsp.payload: { discComplete, attrInfos: [ { attrId, dataType }, ... ] }
                const payload = discoverRsp.payload;
                let discComplete = payload.discComplete;
                let attrInfos = payload.attrInfos;
                let nextReqIndex;
...
function dispatchIncomingMsg(type, msg) {
    // remote ep, or a local ep (maybe a delegator)
    let targetEp;
    let remoteEp;
    // which callback on targetEp
    let dispatchTo;
    let zclHeader;
    // check whether the msg is foundation(0) or functional(1)
    let frameType;
    // bridged event
    let mandatoryEvent;

    if (msg.hasOwnProperty('endpoint')) {
        // dataConfirm, reflectError
        //  => find local ep, such a message is going to local ep
        ...
        // incomingMsg, incomingMsgExt, zclIncomingMsg
        //  => find local ep
        targetEp = af.controller.getCoord().getEndpoint(msg.dstendpoint);

        if (targetEp) {
            remoteEp = af.controller.findEndpoint(msg.srcaddr, msg.srcendpoint);

            // delegator, pass message to remote endpoint
            if (targetEp.isDelegator()) {
                targetEp = remoteEp;
            } else if (!remoteEp) {
                // local zApp not found, get ieeeaddr and emit fake 'endDeviceAnnceInd' msg
                ...
                if (_.isArray(msgBuffer)) {
                    ...
                    const opts = {
                        shortaddr: msg.srcaddr,
                        reqtype: 0,
                        startindex: 0,
                    };

                    af.controller.request('ZDO', 'ieeeAddrReq', opts)
                        .then((rsp) => {
                            ...
                            af.controller.emit('ZDO:endDeviceAnnceInd', {
                                srcaddr: rsp.nwkaddr,
                                nwkaddr: rsp.nwkaddr,
                                ieeeaddr: rsp.ieeeaddr,
                                capabilities: {},
                            });
                        })
                        .fail((err) => {
    ...
    // if target not found, ignore this message
    if (!targetEp) return;

    switch (type) {
        case 'dataConfirm':
            // msg: { status, endpoint, transid }
            // sender(loEp) is listening, see af.send() and af.sendExt()
        ...
        case 'incomingMsg':
            ...
            // a zcl packet maybe, pre-parse it to get the header
        ...
        case 'zclIncomingMsg':
            ...
            if (targetEp.isLocal()) {
                ...
                let localEp = af.controller.findEndpoint(0, msg.dstendpoint);
                let toLocalApp = false;

                if (localEp) {
                    toLocalApp = localEp.isLocal()
                        ? !localEp.isDelegator()
                        : false;
                }
            ...
            if (frameType === 0 && msg.zclMsg.cmdId === 'report') {
                af.controller.getBridge().emit('ind:reported', targetEp, msg.clusterid, msg.zclMsg.payload);
            }
    ...
    if (_.isFunction(dispatchTo)) {
        setImmediate(() => dispatchTo(msg, remoteEp));
    }
    ...
    if (type === 'zclIncomingMsg') return;

    // further parse for ZCL packet from incomingMsg and incomingMsgExt
    if (zclHeader) {
        // if (zclHeader && targetEp.isZclSupported()) {
        if (zclHeader.frameCntl.frameType === 0) {
            // foundation
            zcl.parse(msg.data, function(err, zclData) {
                if (!err) zclIncomingParsedMsgEmitter(msg, zclData);
            });
        } else if (zclHeader.frameCntl.frameType === 1) {
            // functional
...
function makeAfParams(loEp, dstEp, cId, rawPayload, opt) {
    ...
    let afOptions = ZSC.AF.options.ACK_REQUEST | ZSC.AF.options.DISCV_ROUTE;
    let afParams = {
...
function makeAfParamsExt(loEp, addrMode, dstAddrOrGrpId, cId, rawPayload, opt) {
    ...
    let afOptions = ZSC.AF.options.DISCV_ROUTE;
    let afParamsExt = {
    ...
    switch (addrMode) {
        ...
        case ZSC.AF.addressMode.ADDR_64BIT:
            afParamsExt.destendpoint = opt.hasOwnProperty('dstEpId')
                ? opt.dstEpId
                : 0xFF;
            afParamsExt.options = opt.hasOwnProperty('options')
                ? opt.options
...
function nextZclSeqNum() {
    ...
    seqNumber += 1;
    if (seqNumber > 255 || seqNumber < 0) seqNumber = 0;
...
module.exports = function(controller) {
    ...
    function isAttached(evt, lsn) {
        let has = false;
        let lsns = af.controller.listeners(evt);
